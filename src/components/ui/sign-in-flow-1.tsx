  ...
  const handleResendCode = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        // Show success indicator/toast if window.showToast exists
        if ((window as any).showToast) {
          (window as any).showToast("Code resent successfully!", "success");
        }
      }
    } catch (err: any) {
      setError(err.message || "An unexpected error occurred.");
  ...
  const verifyCode = async (tokenString: string) => {
    ...
    try {
      ...
      if (error) {
        ...
        // Successfully verified! Insert or upsert default profile
        if (data?.user) {
          try {
                  ...
                  >
                    <div className="space-y-3 overflow-hidden">
                      <motion.h1 
                        ...
                        className="text-3xl sm:text-4xl font-bold leading-tight tracking-normal text-white"
                      >
                        Land your target developer role with AI.
                      </motion.h1>
                      <motion.p 
                      ...
                      >
                        Track applications, analyze skill compatibility, and generate custom cover letters using Gemini 1.5 Flash.
                      </motion.p>
                    </div>
                    
                    {error && (
                      <div className="p-3 bg-red-950/40 border border-red-500/20 text-red-200 text-sm rounded-full px-5 text-center">
                        {error}
                      </div>
                    )}
                    ...
                    <div className="space-y-4">
                      <button 
                        ...
                        disabled={loading}
                        className="backdrop-blur-[2px] w-full flex items-center justify-center gap-2 bg-white/5 hover:bg-white/10 text-white border border-white/10 rounded-full py-3 px-4 transition-colors cursor-pointer disabled:opacity-55"
                      >
                      ...
                      <form onSubmit={handleEmailSubmit}>
                        <div className="relative">
                          <input 
                            ...
                            disabled={loading}
                            className="w-full backdrop-blur-[1px] text-white bg-transparent border border-white/10 rounded-full py-3 px-4 focus:outline-none focus:border-white/30 text-center disabled:opacity-55"
                            required
                          />
                          <button 
                            type="submit"
                            disabled={loading}
                            className="absolute right-1.5 top-1.5 text-white w-9 h-9 flex items-center justify-center rounded-full bg-white/10 hover:bg-white/20 transition-colors group overflow-hidden cursor-pointer disabled:opacity-55"
                          >
                            {loading ? (
                  ...
                  >
                            ...
                            <div key={i} className="flex items-center">
                              <div className="relative">
                                <input
                                  ...
                                  className="w-8 text-center text-xl bg-transparent text-white border-none focus:outline-none focus:ring-0 appearance-none disabled:opacity-55"
                    ...
                    <div className="flex w-full gap-3">
                      <motion.button 
                        ...
                        disabled={loading}
                        className="rounded-full bg-white text-black font-medium px-8 py-3 hover:bg-white/90 transition-colors w-[30%] cursor-pointer disabled:opacity-55"
                        whileHover={{ scale: 1.02 }}
                        whileTap={{ scale: 0.98 }}
                        transition={{ duration: 0.2 }}
                      >
